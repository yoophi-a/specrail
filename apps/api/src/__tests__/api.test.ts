...
test("API creates and filters tracks by project", async () => {
  await withServer(async (baseUrl) => {
    ...
    const blankProjectTrackResponse = await fetch(`${baseUrl}/tracks`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        projectId: " ",
        title: "Blank project track",
        description: "This should fail validation before project lookup.",
      }),
    });
    assert.equal(blankProjectTrackResponse.status, 422);
    const blankProjectTrackPayload = (await blankProjectTrackResponse.json()) as {
      error: { code: string; details: Array<{ field: string }> };
    };
    assert.equal(blankProjectTrackPayload.error.code, "validation_error");
    assert.deepEqual(
      blankProjectTrackPayload.error.details.map((detail) => detail.field),
      ["projectId"],
    );
...
test("API supports creating tracks, planning sessions, messages, starting runs, and listing run events", async () => {
  await withServer(async (baseUrl, paths) => {
    ...
    const noThreadBindResponse = await fetch(`${baseUrl}/channel-bindings`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        projectId: "project-default",
        ...
        externalUserId: "user-without-thread",
        trackId: trackPayload.track.id,
      }),
    });
    ...
    const missingProjectBindResponse = await fetch(`${baseUrl}/channel-bindings`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        projectId: "project-missing",
        channelType: "telegram",
        externalChatId: "missing-project-chat",
        trackId: trackPayload.track.id,
      }),
    });
    assert.equal(missingProjectBindResponse.status, 404);
    const missingProjectBindPayload = (await missingProjectBindResponse.json()) as {
      error: { code: string; message: string };
    };
    ...
    const missingPlanningSessionBindResponse = await fetch(`${baseUrl}/channel-bindings`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        projectId: "project-default",
        channelType: "telegram",
        externalChatId: "missing-planning-session-chat",
        trackId: trackPayload.track.id,
        planningSessionId: "missing-session",
      }),
    });
    assert.equal(missingPlanningSessionBindResponse.status, 404);
    const missingPlanningSessionBindPayload = (await missingPlanningSessionBindResponse.json()) as {
      error: { code: string; message: string };
    };
    ...
    const missingPlanningSessionAttachmentResponse = await fetch(`${baseUrl}/attachments`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        sourceType: "telegram",
        externalFileId: "missing-planning-session-file",
        planningSessionId: "missing-session",
      }),
    });
    assert.equal(missingPlanningSessionAttachmentResponse.status, 404);
    const missingPlanningSessionAttachmentPayload = (await missingPlanningSessionAttachmentResponse.json()) as {
      error: { code: string; message: string };
    };
...
test("API supports proposing, approving, and rejecting artifact revisions", async () => {
  await withServer(async (baseUrl) => {
    ...
    assert.equal(artifactPayload.approvalRequests[0]?.id, pendingProposal.approvalRequest.id);
    assert.equal(artifactPayload.approvalRequests[0]?.trackId, trackPayload.track.id);
    assert.equal(artifactPayload.approvalRequests[0]?.artifact, "spec");
    assert.equal(artifactPayload.approvalRequests[0]?.revisionId, pendingProposal.revision.id);
    assert.equal(artifactPayload.approvalRequests[0]?.requestedBy, "agent");
    assert.equal(artifactPayload.approvalRequests[0]?.requestedAt, pendingProposal.approvalRequest.requestedAt);
    assert.ok(artifactPayload.approvalRequests[0]?.decidedAt);
    assert.equal(artifactPayload.approvalRequests[0]?.decidedBy, "user");
    assert.equal(artifactPayload.approvalRequests[0]?.decisionComment, "Ship it");
    assert.equal(artifactPayload.approvalRequests[1]?.id, rejectProposal.approvalRequest.id);
    assert.equal(artifactPayload.approvalRequests[1]?.trackId, trackPayload.track.id);
    assert.equal(artifactPayload.approvalRequests[1]?.artifact, "spec");
    assert.equal(artifactPayload.approvalRequests[1]?.revisionId, rejectProposal.revision.id);
    assert.equal(artifactPayload.approvalRequests[1]?.requestedBy, "agent");
    assert.equal(artifactPayload.approvalRequests[1]?.requestedAt, rejectProposal.approvalRequest.requestedAt);
    assert.ok(artifactPayload.approvalRequests[1]?.decidedAt);
    assert.equal(artifactPayload.approvalRequests[1]?.decidedBy, "user");
    assert.equal(artifactPayload.approvalRequests[1]?.decisionComment, "Need more detail");
