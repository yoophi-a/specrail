  ...
  return `const scope = document.querySelector('#project-scope');
    ...
    function renderRunDetail(runPayload, eventsPayload, cleanupPayload) {
      ...
      detail.innerHTML = '<h3>Run ' + escapeHtml(run.id) + '</h3>'
        ...
        + '<h3>Run report</h3><p data-control-group="run-report"><a class="pk-source" data-run-report="' + escapeHtml(run.id) + '" href="/runs/' + encodeURIComponent(run.id) + '/report.md" target="_blank" rel="noopener" download>↗ Open/download Markdown run report</a></p>'
        + cleanupSection
