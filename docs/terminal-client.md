...
- compares the selected revision against the current artifact with changed-line counts and a short +/- preview, with `u` to toggle an expanded all-changed-lines view
...
- add richer revision diff/compare views before approval, such as side-by-side paging or full patch export to a file
