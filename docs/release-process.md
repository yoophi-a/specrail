...
`SPECRAIL_EXECUTION_BACKEND` is validated at startup and accepts `codex`, `claude_code`, or `claude-code`. Run API request body `backend` values use the same normalization before validation.
