...
- `SPECRAIL_TERMINAL_PREFERENCES_PATH`
- `SPECRAIL_TERMINAL_MESSAGE_TEMPLATES_PATH`
- `SPECRAIL_TERMINAL_DIFF_EXPORT_DIR`
- `TELEGRAM_BOT_TOKEN`
- `TELEGRAM_APP_PORT`
...
Track/run list `sortBy` query values accept mixed-case, hyphenated, and underscore aliases such as `updated_at` and `created-at`, then resolve to the canonical camelCase field before validation.
