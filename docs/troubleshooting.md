...
3. The configured default execution backend is supported by the current build. `SPECRAIL_EXECUTION_BACKEND` accepts `codex` and `claude_code`; `claude-code` is normalized to `claude_code`. Run API request body `backend` values follow the same normalization.
