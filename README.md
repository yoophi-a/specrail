...
- `specrail/workspace/read` exposes scoped linked-run workspace reads with sorted directory entries, structured refusal reasons, and cleanup-block metadata
...
- `GET /tracks`
  ...
  - enum query values for `status`, `priority`, and `sortOrder` are normalized for case, `status` accepts hyphenated forms such as `in-progress`, and `sortBy` accepts mixed-case or separator aliases such as `updated_at`
...
- `SPECRAIL_TERMINAL_PREFERENCES_PATH` optional local JSON file for persisting project scope, run filter, refresh interval, live-tail pause, and event-detail visibility changes; persisted `runFilter` values are trimmed and parsed case-insensitively
- `SPECRAIL_TERMINAL_MESSAGE_TEMPLATES_PATH` optional JSON file for custom terminal planning-message templates; template `kind` and `relatedArtifact` enum values are trimmed and parsed case-insensitively
...
For the GitHub webhook app, set `GITHUB_WEBHOOK_SECRET` for real deliveries and optionally `GITHUB_APP_PORT` / `GITHUB_WEBHOOK_PATH` before `pnpm dev:github`. `GITHUB_APP_PORT` must be an integer TCP port in the `0..65535` range, and `GITHUB_WEBHOOK_PATH` is normalized to start with `/` when configured.
